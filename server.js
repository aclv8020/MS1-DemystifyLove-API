...
app.use((req, res, next) => {
    ...
    // res.header('Access-Control-Allow-Origin', 'https://andysellsvegas.com'); //add allowed domain(s)
...
// // Route to handle the POST request from your web page
// app.post('/get-data', async (req, res) => {
//     const apiKey = process.env.API_KEY; // API key stored in .env
//     const apiUrl = 'https://api.mindstudio.ai/developer/v2/agents/run';
...
//                 'Content-Type': 'application/json',
//             },
//             body: JSON.stringify(req.body), // Forward the exact data from your web page
//         });
...
//         const data = await response.json();
//         res.json(data); // Send the API response back to your web page
//     } catch (error) {
//         console.error('Error:', error);
//         res.status(500).json({ error: error.message });
//     }
